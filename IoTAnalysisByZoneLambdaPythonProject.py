...
def lambda_handler(event, context):
    ...
    if(_from !='' and _to!=''):         # from~to 시간 입력이 있으면 필요한 기간의 데이터만 db에서 불러옴
        ...
        response = location_table.scan(
                FilterExpression = Attr('time').between(start_date, end_date) & Attr('device').eq(_device)
            )
    else:
        # DB에서 해당 시간대 데이터 가져옴
        response = location_table.scan(
                FilterExpression = Attr('device').eq(_device)
            )
    
    loc_list = response['Items']
    
    # 해당 시간대에 저장된 위치기록이 없으면 빈 딕셔너리 리턴
    if len(loc_list)==0:
    ...
    while(i<len(loc_list)):   # 위치 배열 크기만큼 반복
        ...
        #print("total_list[remember_zone] : ", total_list["SUYU"])
        #total_list[remember_zone] += stay_time
