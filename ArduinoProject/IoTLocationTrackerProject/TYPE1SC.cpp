...
#define SWIR_TRACE(...) TYPE1SC_trace(__VA_ARGS__);       // ----------------- 이부분 뭐지? 가변인자 매크로?
...
// 도메인을 숫자형 IP 주소로 바꿔주는 듯
// 첫 번째 인자 : 도메인 주소 보냄
// 두 번째 인자 : 숫자형 IP 주소 받아 쓰는 듯
// 세 번째 인자 : *dstAddr(두 번째 인자) 의 크기
int TYPE1SC::getIPAddr(const char *srcAddr, char *dstAddr, int dstAddrSize) {
  char szCmd[128];
  char szRst[128];
  int ret;

  // _Serial에 값이 있으면 읽는 듯
  TYPE1SC_serial_clearbuf();

  // memset(세팅하고자 하는 메모리 시작 주소, 셋팅하고자 하는 값, 길이)
  // 성공 시 첫번째 인자 반환, 실패 시 NULL 반환
  memset(dstAddr, 0, dstAddrSize);

  // sprintf() 함수는 배열 버퍼에 일련의 문자와 값의 형식을 지정하고 저장
  // sprintf(배열, 서식, 값);
  // szCmd => AT%%DNSRSLV=0, "srcAddr"
...
int TYPE1SC::readATresponseLine(char *szLine, int nLineBufSize,
  ...
  do {
    if (_serial.available()) {    // 값이 있으면
      String sStr;
      
      // Serial.readStringUntil(종료문자)
      // 시리얼 버퍼를 종료 문자까지 읽고 읽은 데이터는 지운다
      sStr = _serial.readStringUntil('\n');
      int nLen = sStr.length();
      ...
      if (nLen > 1) {
        ...
        // malloc(사이즈) : 사이즈만큼 동적 할당하고 메모리 시작주소 리턴
        aLine[nbLine] = (char *)malloc(nLen + 1);
        sStr.toCharArray(aLine[nbLine], nLen);
        aLine[nbLine][nLen] = 0;
...
// -------------------------------------------------------------------------------------
int TYPE1SC::AWSIOT_RECV(char *buffer, int bufferSize) {
  char resBuffer[200];
  int ret;

  // 응답 내용이 있으면 resBuffer에 해당 내용 복사하여 가져옴
  // PUBRCV\", 위치 이후부터 읽어들임
  ret = readATresponseLine(resBuffer, sizeof(resBuffer), "PUBRCV\",", 5000);

  if(ret == 0){
    char *ptr = strstr(resBuffer, ",");       // 주제 내용 제외하고 게시 내용만 확인하기 위함
    ptr+=2;                                   // , " 제외
    memset(buffer, 0, bufferSize);
    strcpy(buffer, ptr);          
  }
